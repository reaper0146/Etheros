...
contract Market is Ownable {
    ...
    function sellArticle(string memory _name, string memory _description, uint256 _price, string memory _hashvalue) public {
        ...
        // trigger the event
        emit LogSellArticle(articleCounter, msg.sender, _name, _price, _hashvalue);
    }
    ...
    function buyArticle(uint _id) public payable {
       ...
       // emit GiveHash()
        // trigger the event
        emit LogBuyArticle(_id, article.seller, article.buyer, article.name, article.price, article.hashvalue);
